...
public:
    ...
    static uint32_t pack4(const uint8_t *a) {
        return uint32_t(a[0] << 0*8) | uint32_t(a[1] << 1*8) | uint32_t(a[2] << 2*8) | uint32_t(a[3] << 3*8);
    }
    ...
    CC20(const uint8_t key[32], const uint8_t nonce[8]) {
...
#define CHACHA20_QUARTERROUND(x, a, b, c, d) \
    ...
    static void XOR(unsigned char *bytes, size_t n_bytes, const char *pszKey) {
        CC20 block((uint8_t*)pszKey, (uint8_t*)"7Isr1k9s");
        uint8_t key8[64];
        size_t position = 64;
        block.set_counter(0);
